...
from ctypes import c_ulong, c_char, c_int, c_void_p, c_long, c_char_p, c_uint32, c_void_p, c_int64, c_float, c_bool, c_uint, c_uint8
...
class Frame(Structure):
    ...
    _fields_ = [
    #input
    ("ImageBuffer", POINTER(c_uint8)), #void* // image buffer
    ("ImageBufferSize", c_ulong),
    ("AncillaryBuffer", POINTER(c_uint8)),#void * //buffer to capture associated header and trailer data
...
class Camera(object):
    ...
    def getAttr(self, name):
        ...
        elif dtype == PvDatatype.ePvDatatypeString:
            return self.getStringAttr(name)
        else:
            self._log ("Attr type not implemented yet for %s of type %s:%s " % (name, PvDatatype[dtype], dtype))
            #return self.getStringAttr(name)
    ...
    def setAttr(self, name, val):
        ...
        if dtype == PvDatatype.ePvDatatypeEnum: 
            ...
            raise PvException("Attr type not implemented yet for %s of type %s " % (name, dtype))
    ...
    def getSingleFrame(self):
        ...
        deprecated, 
        """
        self._log("Deprecated, use startCaptureTrigger and then getFrame")
    ...
    def getSingleNumpyArray(self):
        ...
        deprecated
        """
        self._log("Deprecated, use startCaptureTrigger and then getNumpyArray")
        self.startCaptureTrigger()
    ...
    def getNumpyArray(self):
        ...
        for i in range(0, 15): #usually the first frame is ok, but shit happens regularily
            ...
            if frame.ImageSize == self.frameSize:
                im = np.ctypeslib.as_array(frame.ImageBuffer, (frame.height, frame.width))
                ...
                return im.copy()
            self._log("Bad frame from %s, expected size %s, received size %s" % ( self.name + str(self.id), self.frameSize, frame.ImageSize ))
        raise PvException
    ...
    def getStringAttr(self, name):
        size = 40
        att = create_string_buffer(size) 
        realsize = c_long()
        err = self._dll.PvAttrStringGet(self._handle, name, byref(att), size, byref(realsize))
        if realsize.value > size:
            att = create_string_buffer(realsize) 
            err = self._dll.PvAttrStringGet(self._handle, name, byref(att), size, byref(realsize))
    ...
    def getEnumVals(self, name):
        size = c_ulong(40)
        st = create_string_buffer(size.value)
        newsize = c_ulong()
        ret = self._dll.PvAttrRangeEnum(self._handle, name, byref(st), size, byref(newsize))
        if newsize.value > size.value:
            size = c_ulong((newsize.value + 1))
            st = create_string_buffer(size.value)
            ret = self._dll.PvAttrRangeEnum(self._handle, name, byref(st), size, byref(newsize))
        if ret != 0:
            raise PvException("error getting enum range", ret, PvError[ret])
        st = st.value[:size.value]
        return st.split(",")
    ...
    def printAttr(self, name):
        """ conveniance method
        """
        print self.name + "-" + self.serialNumber +  "::" + name + ": " + str(self.getAttr(name))

    def setOptimalPacketSize(self):
        """
        Find optimal packet size for this network
        """
        size = c_ulong(99999) #input is max packet size we want
        ret = self._dll.PvCaptureAdjustPacketSize(self._handle, size)
        return self.getAttr("PacketSize")
